...
func _ready():
        ...
        # v4.2: Premium styling for leave button (match menu style)
        _style_button(leave_button, Color(1.0, 0.4, 0.3))
        leave_button.mouse_entered.connect(func():
                AudioManager.play_ui_hover()
                var t = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
                t.tween_property(leave_button, "scale", Vector2(1.06, 1.06), 0.1))
        leave_button.mouse_exited.connect(func():
                var t = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
                t.tween_property(leave_button, "scale", Vector2(1.0, 1.0), 0.15))
...
# v4.2: Premium button styling (match menu.gd)
