...
## HUD - Giao diện người chơi (v3.1) - Premium UI + Custom Images
## v3.1: Thêm Win.png overlay (hiện khi thắng, biến mất sau khi bảng xếp hạng hiện)
##       Thêm YouDie.png overlay (hiện khi bị tiêu diệt, biến mất khi hồi sinh)
...
@onready var results_restart_btn: TextureButton = $ResultsPanel/ResultsRestartBtn
@onready var results_menu_btn: TextureButton = $ResultsPanel/ResultsMenuBtn
@onready var you_die_overlay: TextureRect = $YouDieOverlay
@onready var win_overlay: TextureRect = $WinOverlay
...
func _ready():
        ...
        # v3.1: Custom image overlays
        if you_die_overlay:
                you_die_overlay.visible = false
        if win_overlay:
        ...
        # v3.1: Touch scale effect for result buttons
        _setup_touch_scale(results_restart_btn)
        _setup_touch_scale(results_menu_btn)

## v3.1: Setup touch scale animation for buttons
func _setup_touch_scale(btn: Control):
        ...
        btn.gui_input.connect(func(event: InputEvent):
                if event is InputEventMouseButton:
                        if event.pressed:
                                _animate_scale(btn, Vector2(1.12, 1.12), 0.1)
                        else:
...
## Hiển thị YouDie.png khi player bị tiêu diệt
func _show_you_die():
        if not you_die_overlay:
                return
        you_die_overlay.visible = true
        you_die_overlay.modulate.a = 0.0
        var tween = create_tween()
        tween.tween_property(you_die_overlay, "modulate:a", 1.0, 0.3).set_trans(Tween.TRANS_QUAD)

## Ẩn YouDie.png khi player hồi sinh
func _hide_you_die():
        if not you_die_overlay:
                return
        var tween = create_tween()
        tween.tween_property(you_die_overlay, "modulate:a", 0.0, 0.3).set_trans(Tween.TRANS_QUAD)
        tween.tween_callback(func():
                if is_instance_valid(you_die_overlay):
                        you_die_overlay.visible = false
        )

## Hiển thị Win.png khi chiến thắng, tự biến mất sau khi bảng xếp hạng hiện
func _show_win_overlay():
        if not win_overlay:
                return
        ...
        var tween = create_tween()
        # Fade in
        tween.tween_property(win_overlay, "modulate:a", 1.0, 0.4).set_trans(Tween.TRANS_QUAD)
        # Hold for 2 seconds
        tween.tween_interval(2.0)
        # Fade out (biến mất khi bảng xếp hạng đã hiện)
        tween.tween_property(win_overlay, "modulate:a", 0.0, 0.8).set_trans(Tween.TRANS_SINE)
        tween.tween_callback(func():
...
func _show_results(winner_name: String, leaderboard: Array):
        game_over_panel.visible = false
        # v3.1: Ẩn YouDie overlay nếu còn hiện
        if you_die_overlay and you_die_overlay.visible:
                _hide_you_die()
        
        results_panel.visible = true
        results_title.text = "KẾT THÚC! %s thắng!" % winner_name
        
        # v3.1: Hiện Win.png nếu player thắng
