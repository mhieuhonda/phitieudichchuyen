...
func _ready():
	...
	team_tab.pressed.connect(func(): _show_tab(1))
	achievement_tab.pressed.connect(func(): _show_tab(2))
	_refresh_topbar()
	_show_tab(0)
...
func _show_tab(tab: int):
	...
	team_panel.visible = (tab == 1)
	achievement_panel.visible = (tab == 2)
	stats_tab.modulate = Color(1.2, 1.2, 1.0) if tab == 0 else Color(0.6, 0.6, 0.6)
	team_tab.modulate = Color(1.2, 1.2, 1.0) if tab == 1 else Color(0.6, 0.6, 0.6)
	achievement_tab.modulate = Color(1.2, 1.2, 1.0) if tab == 2 else Color(0.6, 0.6, 0.6)
	match tab:
		0: _build_stats()
		1: _build_team()
