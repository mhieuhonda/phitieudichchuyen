...
## Menu - Menu chính / Sảnh chờ (v4.2)
## v4.2:
##   - Nút MULTIPLAYER tự kiểm tra đăng nhập — chưa login thì chuyển qua login scene
##     (login scene có nút "Chơi guest" để bỏ qua nếu muốn)
##   - Thêm dòng branding "Game developed by Hieu Louis" dưới cùng
##   - Version label cập nhật v4.2
## v4.1:
...
func _ready():
        ...
        _setup_developer_label()
        AudioManager.play_music("menu")
        TutorialManager.maybe_auto_open_tutorial()

func _setup_developer_label():
        if developer_label:
                developer_label.text = "Game developed by Hieu Louis"
                developer_label.add_theme_color_override("font_color", Color(1.0, 0.85, 0.3, 0.85))
                developer_label.add_theme_color_override("font_shadow_color", Color(0, 0, 0, 0.6))
                developer_label.add_theme_constant_override("shadow_offset_y", 1)
                developer_label.add_theme_constant_override("shadow_outline_size", 2)
...
func _refresh_ui():
        if version_label:
                version_label.text = "v4.4 - Phi Tiêu Dịch Chuyển"
        if new_feature_label:
                new_feature_label.text = "[color=#ffaa00][b]v4.4:[/b][/color] Fix lỗi không phân giải được tên miền server (DNS pre-resolve + IP fallback + HTTPClient direct) · Fix treo 00:00 sau trận (nút Về sảnh chờ + auto-transition + safety timeout) · Bỏ double-EXP bug\n[color=#44ff88][b]Multiplayer:[/b][/color] Vào menu → 🌐 MULTIPLAYER → (đăng nhập) → Tạo phòng → Mời bạn bè → Deathmatch 3 phút + nhận EXP!"
        if play_button:
...
func _on_multiplayer_pressed():
        AudioManager.play_ui_click()
        AudioManager.play_confirm()
        # v4.2: Bắt buộc đăng nhập trước khi vào multiplayer — nếu không, guest sẽ
        # bị blockchat/exp. Chuyển qua login scene, login scene sẽ tự forward sang
        # multiplayer_lobby sau khi đăng nhập OK (xem login.gd -> _on_logged_in -> next_scene).
