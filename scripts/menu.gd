...
func _refresh_ui():
        ...
        if new_feature_label:
                if I18N.is_vi():
                        new_feature_label.text = "[color=#ffaa00][b]v3.8 MỚI:[/b][/color] Pause menu (P/ESC) + Quick Retry (R) + Minimap radar + Boss Phase 2 (50% HP) + Stage transition\n[color=#44ff88][b]Fix:[/b][/color] Bug player darts không bị AI né được (thiếu 'darts' group) + Boss HP HUD sai (10M→12M) + Boss double-laser spawn\n[color=#55aaff][b]UI/UX:[/b][/color] Hit markers + Kill streak UI + Boss HP segments + % label + off-screen arrow + low-HP heartbeat + aim ricochet preview + onboarding hints + stage stats + dart glow power-scale\n[color=#ff4444][b]Perf:[/b][/color] Cache dart count, audio pop fix, pickup cleanup interval"
                else:
                        new_feature_label.text = "[color=#ffaa00][b]v3.8 NEW:[/b][/color] Pause menu (P/ESC) + Quick Retry (R) + Minimap radar + Boss Phase 2 (50% HP) + Stage transition\n[color=#44ff88][b]Fix:[/b][/color] Player darts not dodgeable by AI (missing 'darts' group) + Boss HP HUD mismatch (10M→12M) + Boss double-laser spawn\n[color=#55aaff][b]UI/UX:[/b][/color] Hit markers + Kill streak UI + Boss HP segments + % label + off-screen arrow + low-HP heartbeat + aim ricochet preview + onboarding hints + stage stats + dart glow power-scale\n[color=#ff4444][b]Perf:[/b][/color] Cache dart count, audio pop fix, pickup cleanup interval"
        if play_button:
