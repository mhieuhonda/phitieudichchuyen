...
## MobileControls - Nút bấm ảo cho mobile (v3.3) - Code-based UI
## v3.3: Tất cả nút đều là Button (Godot Control), không còn TextureButton + PNG.
##       StyleBoxFlat tạo visual: Dash cyan, Shield blue, Multi orange, Teleport/Dịch xanh sáng.
...
# Palette for skill buttons
const COL_DASH := Color(0.4, 1.0, 0.9)
const COL_SHIELD := Color(0.5, 0.9, 1.0)
const COL_MULTI := Color(1.0, 0.7, 0.2)
const COL_TELEPORT := Color(0.3, 0.85, 0.95)
const COL_THROW := Color(0.95, 0.45, 0.30)
...
func _ready():
	...
	# Style all buttons
	_style_skill_button(skill_dash_btn, COL_DASH)
	_style_skill_button(skill_shield_btn, COL_SHIELD)
	_style_skill_button(skill_multishot_btn, COL_MULTI)
	_style_skill_button(teleport_btn, COL_TELEPORT)
	_style_skill_button(throw_btn, COL_THROW)
...
func _style_skill_button(btn: Button, accent: Color):
	if not btn:
		return
	...
	normal.shadow_color = Color(accent.r * 0.5, accent.g * 0.5, accent.b * 0.5, 0.4)
	normal.shadow_size = 6
	normal.shadow_offset = Vector2(0, 3)

	var hover = normal.duplicate()
	hover.bg_color = Color(0.10, 0.10, 0.18, 0.92)
...
func _process(_delta):
	...
	if skill_dash_btn and is_instance_valid(skill_dash_btn):
		_dash_rect = skill_dash_btn.get_global_rect().grow(8.0)
		...
		if event.pressed:
			if _teleport_rect.has_point(event.position) and not is_mouse_on_teleport:
				...
				if throw_btn and is_instance_valid(throw_btn):
					...
					tween.tween_property(throw_btn, "modulate", Color(1.0, 0.85, 0.7, 1.0), 0.08)
				get_viewport().set_input_as_handled()
			elif _dash_rect.has_point(event.position) and not is_mouse_on_dash:
