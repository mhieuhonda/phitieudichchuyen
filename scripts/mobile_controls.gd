...
@onready var skill_dash_btn: TextureButton = $SkillDashButton
@onready var skill_shield_btn: TextureButton = $SkillShieldButton
@onready var skill_multishot_btn: TextureButton = $SkillMultishotButton
...
func _handle_touch_event(event: InputEventScreenTouch):
        if event.pressed:
                ...
                if _throw_rect.has_point(event.position) and aim_touch_index == -1 and not is_aiming:
                        ...
                        get_viewport().set_input_as_handled()
                        return
                # Skill buttons (nếu không phải vùng teleport/throw)
