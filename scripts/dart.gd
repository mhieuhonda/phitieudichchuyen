...
func set_direction(dir: Vector2, pwr: float):
        ...
        # v3.8: Glow intensity scale theo power — power cao = glow sáng hơn + đỏ hơn
        if glow_sprite and glow_sprite.visible:
                var glow_alpha = lerp(0.35, 0.85, power)
                var glow_color_r = lerp(0.3, 1.0, power)
                var glow_color_g = lerp(0.8, 0.4, power)
                var glow_color_b = lerp(1.0, 0.2, power)
                glow_sprite.modulate = Color(glow_color_r, glow_color_g, glow_color_b, glow_alpha)
                # Scale glow theo power
                var glow_scale = lerp(0.8, 1.5, power)
