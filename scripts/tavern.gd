...
func _build_quest_item(q: Dictionary, is_active: bool = false) -> PanelContainer:
	...
	if not is_active:
		...
		# Active quest → nút "Vào ải" (chuyển sang stage_select)
		var btn = Button.new()
		btn.text = "⚔ Vào Ải"
		...
		hbox.add_child(btn)
		# Complete button (debug / test): nếu đã kill boss → complete quest
		var done_btn = Button.new()
		done_btn.text = "✓ Hoàn thành"
		done_btn.custom_minimum_size = Vector2(140, 50)
		_style_button(done_btn, Color(0.5, 1.0, 0.5))
		done_btn.pressed.connect(_on_complete_quest.bind(q))
...
func _on_do_quest(q: Dictionary):
	AudioManager.play_ui_click()
	# Vào ải để làm quest — sử dụng stage mode hiện có
	# Bonus: khi hoàn thành ải, check quest completion
	info_label.text = "Vào ải để hoàn thành quest. Sau khi thắng, ấn 'Hoàn thành' để nhận thưởng."
	info_label.modulate = Color(0.7, 0.85, 1.0)
	get_tree().change_scene_to_file("res://scenes/stage_select.tscn")

func _on_complete_quest(q: Dictionary):
	AudioManager.play_ui_click()
	var ok = ProgressionManager.complete_quest(q["id"])
	if ok:
		info_label.text = "✓ Hoàn thành quest: %s! Phần thưởng đã nhận." % q["name"]
		info_label.modulate = Color(0.5, 1.0, 0.5)
	else:
		info_label.text = "✗ Quest không còn active"
		info_label.modulate = Color(1.0, 0.5, 0.3)
	_refresh_topbar()
	_refresh_quests()
