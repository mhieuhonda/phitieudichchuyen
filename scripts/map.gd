...
func _create_obstacles():
        ...
        for i in num_obstacles:
                ...
                if is_round:
                        ...
                        # Visual: dùng Polygon2D vẽ hình tròn thay vì ColorRect vuông
                        ...
                        circle_poly.color = Color(0.3, 0.28, 0.38)
                        obstacle.add_child(circle_poly)
                        # Viền đậm hơn để dễ nhìn
                        var border = Line2D.new()
                        for j in segments:
                                var angle = (j / float(segments)) * TAU
                                border.add_point(Vector2(cos(angle), sin(angle)) * radius)
                        border.add_point(Vector2(cos(0), sin(0)) * radius)
                        border.width = 2.0
                        border.default_color = Color(0.4, 0.38, 0.48)
