...
## Player - Nhân vật người chơi (v1.0)
...
# === KỸ NĂNG CHỦ ĐỘNG ===
# Skill enum được define ở GameManager.Skill để truy cập mọi nơi
var skill_cooldowns: Dictionary = {GameManager.Skill.DASH: 0.0, GameManager.Skill.SHIELD: 0.0, GameManager.Skill.MULTISHOT: 0.0}
var shield_active: bool = false
var shield_timer: float = 0.0
var is_dashing: bool = false
var dash_timer: float = 0.0
var dash_direction: Vector2 = Vector2.ZERO
var multishot_ready: bool = false  # Next throw will be multishot
...
func _physics_process(delta):
    ...
    if not is_dashing:
        ...
        # Dash: giữ velocity theo dash_direction
        var t = 1.0 - (dash_timer / GameManager.skill_dash_duration)
        var speed_mult = 4.0 * (1.0 - t * 0.5)  # giảm dần
        velocity = dash_direction * GameManager.walk_speed * speed_mult
...
# === SKILLS ===
func activate_skill(skill: GameManager.Skill) -> bool:
    if not is_alive:
        return false
    if skill_cooldowns[skill] > 0:
...
func _do_dash() -> bool:
    ...
    if dir == Vector2.ZERO:
        # Dash theo hướng aim hoặc hướng sprite đang nhìn
        dir = aim_direction if aim_direction != Vector2.ZERO else Vector2.RIGHT
    dir = dir.normalized()
    dash_direction = dir
    is_dashing = true
    dash_timer = GameManager.skill_dash_duration
    skill_cooldowns[GameManager.Skill.DASH] = GameManager.skill_dash_cooldown
...
func _do_shield() -> bool:
    shield_active = true
    shield_timer = GameManager.skill_shield_duration
...
func _spawn_dart(direction: Vector2, power: float):
    if multishot_ready:
        ...
        # Bắn nhiều phi tiêu với spread
        var count = GameManager.skill_multishot_dart_count
        var spread = GameManager.skill_multishot_spread
        for i in count:
            var t = (i - (count - 1) / 2.0) / max(1, count - 1)
...
func _check_teleport_kill(pos: Vector2):
    ...
    for ai in ai_players:
        ...
        if dist < GameManager.teleport_kill_radius + ai.current_size:
            ...
            _update_hp_bar()
            _spawn_kill_effect(ai.global_position)
            _spawn_floating_text("+%d" % GameManager.score_per_kill, Color(1.0, 0.9, 0.2), global_position)
...
func _on_dart_hit_player(dart: Node2D, hit_player: Node2D):
    if hit_player.has_method("take_damage_from"):
        ...
        # Nếu dart giết được AI, player được +score/kill/size/heal
        if was_alive and "is_alive" in hit_player and not hit_player.is_alive:
            ...
            current_hp = GameManager.player_hp
            _update_hp_bar()
            _spawn_kill_effect(hit_player.global_position)
            _spawn_floating_text("+%d" % GameManager.score_per_kill, Color(1.0, 0.9, 0.2), global_position)
            _spawn_floating_text("+%d HP" % int(GameManager.player_max_hp * GameManager.heal_percent_on_kill), Color(0.3, 1.0, 0.3), global_position + Vector2(0, -20))
            GameManager.request_screen_shake(8.0, 0.3)
            AudioManager.play_kill()
